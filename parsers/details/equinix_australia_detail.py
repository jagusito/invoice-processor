# parsers/details/equinix_australia_detail.py
"""
Equinix Australia Detail Parser
Similar to Singapore with all three sections: Recurring, Prior Period, One Time
"""
...
def extract_equinix_items(pdf_path: str, header_data: Dict[str, Any]) -> pd.DataFrame:
    """
    Extract line items from Australia format Equinix invoices
    Handles Recurring, Prior Period, and One Time charges
    """
    try:
        print(f"🇦🇺 Australia Parser - Processing: {os.path.basename(pdf_path)}")
        
        # Extract using pdfplumber
        detail_df = _extract_with_pdfplumber_australia(pdf_path)
        ...
        # Add header context
        detail_df = _add_header_context_australia(detail_df, header_data, pdf_path)
        
        print(f"✅ Extracted {len(detail_df)} Australia records")
        return detail_df
        
    except Exception as e:
        print(f"❌ Error in Australia parser: {e}")
        return pd.DataFrame()

def _extract_with_pdfplumber_australia(pdf_path: str) -> pd.DataFrame:
    """
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page_num, page in enumerate(pdf.pages, start=1):
            ...
            for table_num, table in enumerate(tables, start=1):
                ...
                # Detect Australia column mapping
                column_mapping = _detect_australia_columns(table, header_row)
                if not column_mapping:
                    continue
                
                # Process data rows
                data_rows = table[header_row+1:]
                transactions = _process_australia_rows(data_rows, column_mapping, section_type)
...
def _detect_australia_columns(table: list, header_row: int) -> Dict[str, int]:
    """
    Detect Australia column positions - handle both MRC and NRC columns
    """
    ...
    print(f"    🔍 Australia headers: {header_clean}")
    
    mapping = {}
    
    # Map columns by header text
    for i, header_text in enumerate(header_clean):
        if 'SO#' in header_text or 'BILLING' in header_text:  # Handle SO#/Billing Agreement
            ...
            mapping['mrc_idx'] = i
            print(f"        🔍 Found MRC column at index {i}")
        elif header_text == 'NRC':
            mapping['nrc_idx'] = i
            print(f"        🔍 Found NRC column at index {i}")
    ...
    print(f"    📋 Australia mapping: {mapping}")
    
    # Validate required fields
    required = ['billing_agreement_idx', 'description_idx', 'total_idx']
    ...
    if missing:
        ...
        print(f"    ✅ All required columns found, amount column available")
        return mapping

def _process_australia_rows(data_rows: list, column_mapping: Dict[str, int], section_type: str) -> list:
    """
    Process Australia data rows for any section type
    """
    transactions = []
    ...
    for row in data_rows:
        ...
        # Extract transaction
        transaction = _extract_australia_transaction(row, column_mapping, section_type)
        if transaction:
            transactions.append(transaction)
            print(f"      ✅ Extracted {section_type} line {line_value}: A${transaction['amount']:,.2f}")
    
    return transactions

def _extract_australia_transaction(row: list, mapping: Dict[str, int], section_type: str) -> Dict[str, Any]:
    """
    Extract Australia transaction from row
    """
    ...
    def safe_numeric(idx, default=0.0):
        if idx is None or idx >= len(row) or not row[idx]:
            return default
        try:
            clean = str(row[idx]).replace(',', '').replace('$', '').replace('A$', '').replace('AUD', '').strip()
    ...
    if tax == 0 and amount > 0 and tax_pct > 0:
        ...
        print(f"        💰 Calculated Australia GST: A${amount:,.2f} × {tax_pct}% = A${tax:.2f}")
...
def _add_header_context_australia(detail_df: pd.DataFrame, header_data: Dict[str, Any], pdf_path: str) -> pd.DataFrame:
    """
    Add header context for Australia
    """
    ...
    detail_df['vendor_name'] = header_data.get('vendor', 'Equinix Australia Pty Ltd')
    detail_df['currency'] = 'AUD'
    ...
    detail_df['regional_variant'] = 'equinix_australia'
